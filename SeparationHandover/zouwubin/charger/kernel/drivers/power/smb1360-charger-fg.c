...
struct smb1360_chip {
	...
	struct regulator		*vcc_i2c;	//zouwubin add for i2c_4 power off 2016.03.01
						...
						int reason, bool enable)
{
	...
	/* modify for paralle charging zouwubin 2016.03.17
	 * the original logic is (*disabled_status && !disabled) and (!(*disabled_status) && disabled)
	 * however, the initialise of chip->parallel_chg_disable_status is always PARALLEL_CURRENT ,
	 * so if the device power on and not in parallel charge,
	 * the value of *disabled_status will set as false but not PARALLEL_CURRENT,
	 * and then the logic will come to chaos and the disabled_status should be removed
	 */
	if (!disabled)
...
/* modify for paralle charging zouwubin 2016.03.17
 * these code is cut from smb1360_parallel_work
 * in original design, the smb1360_parallel_work should be a schedule work
 * but in fact there is no schedule work 
 * and I put the schedule check into smb1360_set_appropriate_usb_current
 * so I need to extract these code as a function for code reuse
 */
static void smb1360_parallel_setting(struct smb1360_chip *chip)
{
	u8 reg;
	int rc, i;
	...
	/* It cost some time for charger come to AICL_LIMIT_1500MA, so it will false at the first time. 
	 * it should be use schedule_delayed_work(&chip->parallel_work, VINMIN_DELAY);
	 * to check whether the status of AICL is suit for parallel charge.
	 * Because I havn't check the effect of add a schedule_delayed_work
	 * so I change the logic of ((reg & AICL_CURRENT_STATUS_MASK) == AICL_LIMIT_1500MA) first
	 * if there is no effect, I will change it back again.
	 */
...
static int smb1360_set_appropriate_usb_current(struct smb1360_chip *chip)
{
	...
	if (current_ma <= CURRENT_100_MA) {
		...
		/* only in AC mode need enable parallel charger, add by zouwubin 2016.03.17 */
		if (chip->parallel_charging) {
			smb1360_parallel_setting(chip);
		}
	}
...
#define SMB_I2C_VTG_MIN_UV 1800000		//zouwubin add for i2c_4 power off 2016.03.01
				...
				const struct i2c_device_id *id)
{
	...
	if (chip->vcc_i2c) {
		if (regulator_count_voltages(chip->vcc_i2c) > 0) {
			...
			if (rc) {
				dev_err(&client->dev,
				"zwb regulator vcc_i2c set failed, rc = %d\n", rc);
				return rc;
			}
		}
...
err_set_vtg_i2c:	//zouwubin add for i2c_4 power off 2016.03.01
	if (chip->vcc_i2c)
		if (regulator_count_voltages(chip->vcc_i2c) > 0)
