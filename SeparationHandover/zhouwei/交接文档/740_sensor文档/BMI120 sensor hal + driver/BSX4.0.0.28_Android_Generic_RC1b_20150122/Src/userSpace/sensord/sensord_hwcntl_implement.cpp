...
#define BMI160_ACC_RESOLUTION (0.061) //16bit ADC, default range +-2000 mg. algorithm input requires "mg"
#define BMA255_ACC_RESOLUTION (0.97656) //12bit ADC, default range +-2000 mg. algorithm input requires "mg"
#define BMA255_ACC_BANDWITH_BITS 12
#define BMA250E_ACC_BANDWITH_BITS 10
#define BMA222E_ACC_BANDWITH_BITS 8
#define BMA280_ACC_BANDWITH_BITS 14
...
uint32_t ap_get_sensorlist(struct sensor_t const** p_sSensorList)
{
    ...
    if (0 == bst_sensorlist.list_len)
    {
        ...
        if(SOLUTION_ECOMPASS == solution_type)
        {
        ...
        }else if(sensors_mask)
        {
            avail_sens_regval = sensors_mask;
        }else
        {
            switch (solution_type) {
                case SOLUTION_MDOF:
                    avail_sens_regval = 0x23C20000001DCE1E;
                    break;
                case SOLUTION_ECOMPASS:
                    avail_sens_regval = 0x23C20000001C4E0E;
                    break;
                case SOLUTION_IMU:
                    avail_sens_regval = 0x23C20000000D8E1A;
                    break;
                case SOLUTION_M4G:
                    avail_sens_regval = 0x23C20000001D4E1E;
                    break;
                case SOLUTION_ACC:
                    avail_sens_regval = 0x23C20000000C0002;
                    break;
                default:
                    break;
            }
...
static int64_t bma2x2_total_frm = 0;
static int32_t bma2x2_if_start_up = 0;
#define BMA2x2_RESET_TMESTIMATE() \
{\
    bma2x2_poll_start_tm = android::elapsedRealtimeNano();\
...
static int64_t bmg160_total_frm = 0;
static int32_t bmg160_if_start_up = 0;
#define BMG160_RESET_TMESTIMATE() \
{\
    bmg160_poll_start_tm = android::elapsedRealtimeNano();\
    ...
    }else if(ACC_CHIP_BMA2x2 == accl_chip)
    {
        if (SAMPLE_RATE_DISABLED == sample_rate)
        {
            if (1 == is_acc_open)
            {
                ...
                ret = wr_sysfs_str("fifo_mode", acc_input_dir_name, "0"); // by_pass mode

                sprintf(str_buf, "%d", SENSOR_PM_SUSPEND);
                ret = wr_sysfs_str("op_mode", acc_input_dir_name, str_buf);
        ...
        }else
        {
            ...
            sprintf(str_buf, "%u", bandwidth);
            ret = wr_sysfs_str("bandwidth", acc_input_dir_name, str_buf);

            /*activate is included*/
            if (0 == is_acc_open)
            {
                sprintf(str_buf, "%d", SENSOR_PM_NORMAL);
                ret = wr_sysfs_str("op_mode", acc_input_dir_name, str_buf);

                ret = wr_sysfs_str("fifo_mode", acc_input_dir_name, "2"); // stream mode
    ...
    }else if(GYR_CHIP_BMG160 == gyro_chip)
    {
        if (SAMPLE_RATE_DISABLED == sample_rate)
        {
            if (1 == is_gyr_open)
            {
                ...
                ret = wr_sysfs_str("fifo_mode", gyr_input_dir_name, "0"); // by_pass mode

                sprintf(str_buf, "%d", SENSOR_PM_SUSPEND);
                ret = wr_sysfs_str("op_mode", gyr_input_dir_name, str_buf);
        ...
        }else
        {
            ...
            sprintf(str_buf, "%u", bandwidth);
            ret = wr_sysfs_str("bandwidth", gyr_input_dir_name, str_buf);
            ...
            if (0 == is_gyr_open)
            {
                ...
                ret = wr_sysfs_str("fifo_mode", gyr_input_dir_name, "2"); // stream mode
    ...
    }else if(MAG_CHIP_BMM150 == magn_chip || MAG_CHIP_AKM09912 ==  magn_chip || MAG_CHIP_AKM09911 ==  magn_chip)
    {
        if (SAMPLE_RATE_DISABLED == sample_rate)
        {
            if (1 == is_mag_open)
            {
                ...
                sprintf(str_buf, "%d", SENSOR_PM_SUSPEND);
                ret = wr_sysfs_str("op_mode", mag_input_dir_name, str_buf);
        ...
        }else
        {
            ...
            if (0 == is_mag_open)
            {
                ...
                if(MAG_CHIP_BMM150 == magn_chip){
                    ...
                    ret = wr_sysfs_str("rept_xy", mag_input_dir_name, "4");
                    ret = wr_sysfs_str("rept_z", mag_input_dir_name, "15");
                }
...
static int32_t magn_offset_open_cnt = 0;
static int32_t gyro_offset_open_cnt = 0;
...
static void ap_send_config(int32_t bsx_list_inx)
{
    ...
    ALSH_SENSOR_CONFIG *p_config;
    bsx_sensor_configuration_t bsx_config_output[2];
    int32_t bsx_supplier_id;
    bsx_u32_t vir_sensor_config_cnt;
    bsx_u32_t phy_sensor_config_cnt;
    int32_t list_inx_base;
    uint32_t i;
    bsx_return_t bsx_ret;
    bsx_sensor_configuration_t bsx_config_input[4] = {
            { BSX_PHYSICAL_SENSOR_ID_INVALID, 0.0f },
            { BSX_PHYSICAL_SENSOR_ID_INVALID, 0.0f },
            { BSX_PHYSICAL_SENSOR_ID_INVALID, 0.0f },
            { BSX_PHYSICAL_SENSOR_ID_INVALID, 0.0f },
    };
    ...
    if (bsx_list_inx <= SENSORLIST_INX_AMBIENT_IAQ)
    {
        ...
        p_config = alsh_sensor_config_nonwk;
    }
    else
    {
        list_inx_base = SENSORLIST_INX_WAKEUP_SIGNI_PRESSURE;
        p_config = alsh_sensor_config_wk;
    }
    ...
    /**
     * In library BSX4, MAG/GYRO OFFSET is shared by non-wakeup Uncal MAG/GYRO and wakeup Uncal MAG/GYRO
     * So HAL has to maintain the open counter of MAG/GYRO OFFSET and handle open/close OFFSET & Uncal accordingly
     */
    if(BSX_OUTPUT_ID_MAGNETICFIELD_RAW == bsx_supplier_id ||
            BSX_WAKEUP_ID_MAGNETICFIELD_RAW == bsx_supplier_id)
    {
        if(0 == magn_offset_open_cnt)
        {
            vir_sensor_config_cnt = 2;
            ...
            bsx_config_output[1].sensor_id = BSX_OUTPUT_ID_MAGNETICFIELD_OFFSET;
            bsx_config_output[1].sample_rate = 0;
        }else{
            vir_sensor_config_cnt = 1;

            bsx_config_output[0].sensor_id = bsx_supplier_id;
            CONVERT_DATARATE_CODE(p_config[bsx_list_inx - list_inx_base].data_rate, bsx_config_output[0].sample_rate);
        }

        magn_offset_open_cnt++;

    }else if(BSX_OUTPUT_ID_ANGULARRATE_RAW == bsx_supplier_id ||
            BSX_WAKEUP_ID_ANGULARRATE_RAW == bsx_supplier_id)
    {
        if(0 == gyro_offset_open_cnt)
        {
            vir_sensor_config_cnt = 2;

            bsx_config_output[0].sensor_id = bsx_supplier_id;
            CONVERT_DATARATE_CODE(p_config[bsx_list_inx - list_inx_base].data_rate, bsx_config_output[0].sample_rate);
            bsx_config_output[1].sensor_id = BSX_OUTPUT_ID_ANGULARRATE_OFFSET;
            bsx_config_output[1].sample_rate = 0;
        }else{
            vir_sensor_config_cnt = 1;
        ...
        gyro_offset_open_cnt++;

    }else /*others are handled in common process*/
    {
        vir_sensor_config_cnt = 1;
    ...
    if(!algo_pass)
    {
        /* update the subscription of library outputs */
        phy_sensor_config_cnt = ARRAY_ELEMENTS(bsx_config_input);
        bsx_ret = sensord_update_subscription(bsx_config_output, &vir_sensor_config_cnt,
                bsx_config_input, &phy_sensor_config_cnt, active_nonwksensor_cnt + active_wksensor_cnt);
        if (BSX_OK != bsx_ret)
        {
            PERR("sensord_update_subscription fail, ret = %d", bsx_ret);
            return;
        }

        for (i = 0; i < phy_sensor_config_cnt; i++)
        {
            ap_config_physensor(bsx_config_input[i].sensor_id, bsx_config_input[i].sample_rate);
        }
...
static void ap_send_disable_config(int32_t bsx_list_inx)
{
    ...
    if(BSX_OUTPUT_ID_MAGNETICFIELD_RAW == bsx_supplier_id ||
            BSX_WAKEUP_ID_MAGNETICFIELD_RAW == bsx_supplier_id)
    {
        if(1 == magn_offset_open_cnt)
        {
            vir_sensor_config_cnt = 2;

            bsx_config_output[0].sensor_id = bsx_supplier_id;
            bsx_config_output[0].sample_rate = SAMPLE_RATE_DISABLED;
            bsx_config_output[1].sensor_id = BSX_OUTPUT_ID_MAGNETICFIELD_OFFSET;
            bsx_config_output[1].sample_rate = SAMPLE_RATE_DISABLED;
        }else{
            vir_sensor_config_cnt = 1;
...
static void ap_hw_process_IIO_frame(int32_t header, char *data, BstSimpleList *dest_list)
{
    ...
    switch (header)
    {
        case FIFO_HEAD_A:
            ...
            p_hwdata->x = (float) rawdata_tmp * BMI160_ACC_RESOLUTION;

            memcpy(&rawdata_tmp, data + 2, sizeof(rawdata_tmp));
            p_hwdata->y = (float) rawdata_tmp * BMI160_ACC_RESOLUTION;

            memcpy(&rawdata_tmp, data + 4, sizeof(rawdata_tmp));
            p_hwdata->z = (float) rawdata_tmp * BMI160_ACC_RESOLUTION;
...
static void ap_hw_poll_bma2x2(BstSimpleList *dest_list)
{
    ...
    if(-1 == fifo_fd){
        PERR("can not open %s, errno = %d", fname_buf, errno);
        return;
    }
    ...
    close(fifo_fd);
    if(ret < 0 ){
        PERR("read %d fail: ret = %d, errno = %d", fifo_fd, ret, errno);
        return;
    }

    frames = ret/6;/*2BytesX 2BytesY 2BytesZ*/
    /*there's a start up time needed for BMA2x2*/
    if(0 == bma2x2_if_start_up && frames < bma2x2_expected_frms)
    {
    ...
    for (i = 0; i < frames; ++i)
    {
        ...
        X_tmp >>= (16 - BMA255_ACC_BANDWITH_BITS);
        Y_tmp = fifo_data_buf[i*6+2] | (fifo_data_buf[i*6+3] << 8);
        Y_tmp >>= (16 - BMA255_ACC_BANDWITH_BITS);
        Z_tmp = fifo_data_buf[i*6+4] | (fifo_data_buf[i*6+5] << 8);
        Z_tmp >>= (16 - BMA255_ACC_BANDWITH_BITS);
        ...
        p_hwdata->id = SENSOR_TYPE_ACCELEROMETER;
        p_hwdata->x = X_tmp * BMA255_ACC_RESOLUTION;
        p_hwdata->y = Y_tmp * BMA255_ACC_RESOLUTION;
        p_hwdata->z = Z_tmp * BMA255_ACC_RESOLUTION;
...
{
#define MIN_INTERVAL_ALLOW 4000000
    int32_t ret;
    int32_t fifo_fd = -1;
    static char fname_buf[MAX_FILENAME_LEN+1] = {0};
    ...
    if(0 == bmg160_if_start_up && frames < bmg160_expected_frms)
    {
        /*update time stamp estimation base line*/
    ...
    for (i = 0; i < frames; ++i)
    {
        ...
        if(p_hwdata->timestamp > sys_tm)
        {
            if(sys_tm - bmg160_last_frm_tm > MIN_INTERVAL_ALLOW)
            {
                bmg160_last_frm_tm = sys_tm;
                p_hwdata->timestamp = sys_tm;
                /*from log we can see that there still frames behind sometimes, so add current one and drop the left*/
                hw_remap_sensor_data(&(p_hwdata->x_uncalib), &(p_hwdata->y_uncalib), &(p_hwdata->z_uncalib), g_place_g);
...
static void ap_hw_poll_bmm150(BstSimpleList *dest_list)
{
    ...
    sprintf(str_buf, "%d", SENSOR_PM_LP1); //force update mag sample, otherwise it update only in 10Hz
    ret = wr_sysfs_str("op_mode", mag_input_dir_name, str_buf);
    ...
    fclose(value_fp);
    if(ret <= 0){
        PERR("fscanf fail, errno = %d", errno);
        return;
    }
...
#define MAX_TMBUF_LEN 20
    ...
    {
        /* What observed from log, there're 2 possibilities:
         * 1) Because BMI160 WM_INT handler could be occasionally delayed,
         * so that A/G samples doubled in one WM_INT period, also the timestamps were
         * wrong calculated. Then MAG samples polled were also doubled to be 2.
         * 2) When BMI160 start to work, the MAG list could have historical samples and can't
         * be synchronized with BMI160 samples. In BMI160 FIFO, we can see that it drop the historical
         * Mag samples
         *
         * To simply workaround this occasional scenario
         * For scenario 1), just drop MAG samples. It should be sufficient to drop only one sample.
         * For scenario 2), drop all MAG samples (In this scenario, tm of MAG sample is less than G/A sample,
         * but that should not occur normally).
         */
        while(1 != ml->list_len){
            /*scenario 1*/
        ...
        /*check if it's scenario 2*/
        MAG_p_hwdata = (HW_DATA_UNION *)(ml->head->p_data);

        if(gl->list_len){
            base_p_hwdata = (HW_DATA_UNION *)(gl->head->p_data);
        }else if(al->list_len){
            base_p_hwdata = (HW_DATA_UNION *)(al->head->p_data);
        }

        if(MAG_p_hwdata->timestamp < base_p_hwdata->timestamp){
            /*M sample tm is still less than G/A samples, it's scenario 2*/
            ml->list_get_headdata((void **)&MAG_p_hwdata);
            free(MAG_p_hwdata);
            return;
        }
    ...
    if(loop > MAX_TMBUF_LEN)
    {
        ...
        for (i = 0; i < loop; ++i)
        {
            base_p_hwdata = (HW_DATA_UNION *)(base_node->p_data);
            if(ABS(base_p_hwdata->timestamp - MAG_p_hwdata->timestamp) < MAX_TM_DEVI ||
                    base_p_hwdata->timestamp > MAG_p_hwdata->timestamp)
            {
    ...
    }else
    {
        ...
        {
            base_p_hwdata = (HW_DATA_UNION *)(base_node->p_data);
            base_tm_buf[i] = base_p_hwdata->timestamp;
            base_node = base_node->next;
        }

        for (i = 0; i < loop; ++i)
        {
            if((0 == i && MAG_p_hwdata->timestamp <= base_tm_buf[i]) ||
                    (i == loop - 1 && MAG_p_hwdata->timestamp > base_tm_buf[i]))
            {
                MAG_p_hwdata->timestamp = base_tm_buf[i];
                break;
            }else if(MAG_p_hwdata->timestamp <= base_tm_buf[i] && MAG_p_hwdata->timestamp > base_tm_buf[i-1])
            {
                if(MAG_p_hwdata->timestamp - base_tm_buf[i-1] < base_tm_buf[i] - MAG_p_hwdata->timestamp)
                {
                    MAG_p_hwdata->timestamp = base_tm_buf[i-1];
                }else
                {
                    MAG_p_hwdata->timestamp = base_tm_buf[i];
                }
...
{
    bsx_version_t bsx_version;
    bsx_return_t bsx_ret;
    const char* solution_name = NULL;

    bsx_ret = bsx_get_version(&bsx_version);
    if (BSX_OK != bsx_ret)
    {
        PWARN("bsx_get_version() fail, ret = %d", bsx_ret);
        memset(&bsx_version, 0, sizeof(bsx_version));
    }
...
* solution type: %s\n \
*****************************",
            bsx_version.version_major, bsx_version.version_minor, bsx_version.bugfix_major, bsx_version.bugfix_minor,
            HAL_ver[0], HAL_ver[1], HAL_ver[2], HAL_ver[3],
            __DATE__, __TIME__, solution_name);
