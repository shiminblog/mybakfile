...
BstSensor::BstSensor()
{
    ...
    shmem_HALdata.p_list = new BstSimpleList();
    pthread_mutex_init(&shmem_HALdata.mutex, NULL);
    pthread_cond_init(&shmem_HALdata.cond, NULL);
...
BstSensor::~BstSensor()
{
    ...
    pthread_mutex_destroy(&shmem_HALdata.mutex);
    pthread_cond_destroy(&shmem_HALdata.cond);
    delete shmem_HALdata.p_list;
 ...
 * @return
 */
int BstSensor::poll_events(sensors_event_t* data, int count)
{
    int ret;
    int msg_cnt = 0;
    int i;
    sensors_event_t* p_listdata;

    pthread_mutex_lock(&(shmem_HALdata.mutex));

    /*poll is blocked in Android specification*/
    if(0 == shmem_HALdata.p_list->list_len)
    {
        ret = pthread_cond_wait(&(shmem_HALdata.cond), &(shmem_HALdata.mutex));
        if (ret)
        {
            pthread_mutex_unlock(&(shmem_HALdata.mutex));
            return 0;
        }
    }

    msg_cnt = MIN(shmem_HALdata.p_list->list_len, (uint32_t)count);

    //PNOTE("### poll off %d events, left in share memory: %d", msg_cnt, shmem_HALdata.p_list->list_len - msg_cnt);

    for (i = 0; i < msg_cnt; i++)
    {
        shmem_HALdata.p_list->list_get_headdata((void **) &p_listdata);
        memcpy(&data[i], p_listdata, sizeof(sensors_event_t));
        free(p_listdata);
    }
