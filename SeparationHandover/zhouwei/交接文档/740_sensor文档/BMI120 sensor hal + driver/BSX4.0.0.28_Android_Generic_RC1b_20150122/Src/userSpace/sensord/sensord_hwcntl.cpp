 ...
 * @date         "Wed Jan 13 14:34:22 2016 +0800"
 * @commit       "8c4b8d6"
 *
 * @brief
 *
...
struct sensor_t bst_all_sensors[SENSORLIST_INX_END] = {
...
, //power consumption, not used
	...
	UNUSED_SENSOR_T("BOSCH signi pressure wakeup")
,
	...
	{	.name = "BOSCH relative humidity wakeup",
		.vendor = "Bosch",
		.version = 1,
...
, //Power Consumption Wakeup, not used
	...
	{	.name = "BOSCH Activity Recognition Sensor", /** to test AR in sensor HAL, name is pre-defined by test app */
		...
		.resolution = 1.0f,
		.power = 0.0f,
		.minDelay = SENSOR_MAXDELAY_ONCHANGE,
		.fifoReservedEventCount = 0,
		.fifoMaxEventCount = 0,
	...
	UNUSED_SENSOR_T("BOSCH ambient co2 wakeup")
,
	UNUSED_SENSOR_T("BOSCH ambient IAQ wakeup")
,
};

BST_SENSORLIST bst_sensorlist = { NULL, NULL, 0 };
...
void get_sensor_t(int32_t sensor_id, struct sensor_t **pp_sensor_t, int32_t *p_list_inx)
{
    ...
    for (i = 0; i < bst_sensorlist.list_len; i++)
    {
        if (NULL == bst_sensorlist.list[i].name)
        {
        ...
        {
            if (sensor_id == bst_sensorlist.list[i].handle)
            {
                p_sensor = &(bst_sensorlist.list[i]);
                break;
            }
        }
    }

    if (i != bst_sensorlist.list_len)
    {
        *pp_sensor_t = p_sensor;
        *p_list_inx = bst_sensorlist.bsx_list_index[i];
    }
    else
    {
        /*not found*/
...
//map with enum BSX4_SENSORLIST_INX
ALSH_SENSOR_CONFIG alsh_sensor_config_nonwk[SENSORLIST_INX_AMBIENT_IAQ+1] = {

	DEFAULT_SENSOR_CONFIG( 0, 0, 0, 4, 0)
,    //SENSORLIST_INX_GAS_RESIST
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_50Hz, ALSH_CONFSTR_UNITms, 200, 7, 0)
...
,    //SENSORLIST_INX_GYROSCOPE,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_50Hz, ALSH_CONFSTR_UNITms, 200, 2, 5)
,    //SENSORLIST_INX_LIGHT,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_50Hz, ALSH_CONFSTR_UNITms, 200, 3, 0)
,    //SENSORLIST_INX_PRESSURE,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_1Hz, ALSH_CONFSTR_UNITms, 200, 2, 5)
,    //SENSORLIST_INX_TEMPERATURE,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_50Hz, ALSH_CONFSTR_UNITms, 200, 2, 0)
,    //SENSORLIST_INX_PROXIMITY,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_50Hz, ALSH_CONFSTR_UNITms, 200, 7, 0)
,    //SENSORLIST_INX_GRAVITY,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_50Hz, ALSH_CONFSTR_UNITms, 200, 7, 0)
,    //SENSORLIST_INX_LINEAR_ACCELERATION,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_50Hz, ALSH_CONFSTR_UNITms, 200, 9, 0)
,    //SENSORLIST_INX_ROTATION_VECTOR,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_1Hz, ALSH_CONFSTR_UNITms, 200, 2, 5)
,    //SENSORLIST_INX_RELATIVE_HUMIDITY,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_1Hz, ALSH_CONFSTR_UNITms, 200, 2, 5)
,    //SENSORLIST_INX_AMBIENT_TEMPERATURE,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_50Hz, ALSH_CONFSTR_UNITms, 200, 6, 0)
,    //SENSORLIST_INX_MAGNETIC_FIELD_UNCALIBRATED,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_50Hz, ALSH_CONFSTR_UNITms, 200, 8, 0)
...
,    //SENSORLIST_INX_STEP_DETECTOR,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_50Hz, ALSH_CONFSTR_UNITms, 200, 4, 5)
,    //SENSORLIST_INX_STEP_COUNTER,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_50Hz, ALSH_CONFSTR_UNITms, 200, 9, 0)
,    //SENSORLIST_INX_MAGNETIC_ROTATION_VECTOR,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_50Hz, ALSH_CONFSTR_UNITms, 200, 1, 5)
...
,    //SENSORLIST_INX_AMBIENT_CO2,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_1Hz, ALSH_CONFSTR_UNITms, 0, 2, 5)
, //SENSORLIST_INX_AMBIENT_IAQ,
};

/*more one NULL pointer for end*/
ALSH_SENSOR_CONFIG * alsh_active_confref_nonwk[SENSORLIST_INX_AMBIENT_IAQ + 1] = { NULL };

//map with enum BSX4_SENSORLIST_INX
//include a TIMESTAMP
ALSH_SENSOR_CONFIG alsh_sensor_config_wk[SENSORLIST_INX_END-SENSORLIST_INX_WAKEUP_SIGNI_PRESSURE] = {
...
,    //SENSORLIST_INX_WAKEUP_AMBIENT_CO2,
	DEFAULT_SENSOR_CONFIG( ALSH_CONFSTR_1Hz, ALSH_CONFSTR_UNITms, 0, 2, 0)
, //SENSORLIST_INX_WAKEUP_AMBIENT_IAQ,
};

/*more one NULL pointer for end*/
ALSH_SENSOR_CONFIG * alsh_active_confref_wk[SENSORLIST_INX_END - SENSORLIST_INX_WAKEUP_SIGNI_PRESSURE] = { NULL };

uint8_t alsh_convert_datarate(int64_t sampling_period_ns)
{
    float Hz;
    ...
    if (Hz > 200)
    {
        return ALSH_CONFSTR_400Hz;
    }
    if (Hz > 100 && Hz <= 200)
    {
        return ALSH_CONFSTR_200Hz;
    }
    if (Hz > 50 && Hz <= 100)
    {
        return ALSH_CONFSTR_100Hz;
    }
    if (Hz > 25 && Hz <= 50)
    {
        return ALSH_CONFSTR_50Hz;
    }
    if (Hz > 12 && Hz <= 25)
    {
        return ALSH_CONFSTR_25Hz;
    }
    if (Hz > 6 && Hz <= 12)
    {
        return ALSH_CONFSTR_12_5Hz;
    }
    if (Hz > 1 && Hz <= 6)
    {
        return ALSH_CONFSTR_6_25Hz;
    }
    if (Hz > 0.5 && Hz <= 1)
    {
        return ALSH_CONFSTR_1Hz;
    }
    if (Hz > 0.25 && Hz <= 0.5)
    {
        return ALSH_CONFSTR_0_5Hz;
    }
    if (Hz > 0.125 && Hz <= 0.25)
    {
        return ALSH_CONFSTR_0_25Hz;
    }
    if (Hz > 0.0625 && Hz <= 0.125)
    {
        return ALSH_CONFSTR_0_125Hz;
    }
    if (Hz > 0.03125 && Hz <= 0.0625)
    {
        return ALSH_CONFSTR_0_0625Hz;
    }
    if (Hz > 0.015625 && Hz <= 0.03125)
    {
        return ALSH_CONFSTR_0_03125Hz;
    }
    if (Hz <= 0.015625)
    {
        return ALSH_CONFSTR_0_015625Hz;
    }

    return 0;
}

void alsh_convert_max_latency(int64_t max_report_latency_ns, uint16_t *value, uint8_t *unit)
{
...
int activate_configref_resort(int32_t bsx_list_index, int32_t is_enable)
{
    ...
    ALSH_SENSOR_CONFIG **p_config_refers;
    ...
    if (bsx_list_index <= SENSORLIST_INX_AMBIENT_IAQ)
    {
        ...
        p_config_refers = alsh_active_confref_nonwk;
    ...
    else
    {
        ...
        p_config_refers = alsh_active_confref_wk;
...
int batch_configref_resort(int32_t bsx_list_index, int64_t sampling_period_ns, int64_t max_report_latency_ns, float delay_Hz)
{
    ...
    for (i = 0; i < (*p_active_sensor_cnt); i++)
    {
        ...
        if (bsx_list_index == current_bsxindex)
        {
            p_config_refers[i]->data_rate = alsh_convert_datarate(sampling_period_ns);
            alsh_convert_max_latency(max_report_latency_ns,
    ...
    p_config[bsx_list_index - bsx_listinx_base].data_rate = alsh_convert_datarate(sampling_period_ns);
    alsh_convert_max_latency(max_report_latency_ns,
...
void *hwcntl_main(void *arg)
{
    ...
    if(bst_sensor->pfun_hw_deliver_sensordata)
    {
        while (1)
        {
