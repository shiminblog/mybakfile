		...
		const struct i2c_device_id *i2c_dev_id)
{
	...
	accl_iio_private = iio_allocate_device(sizeof(*bmi160_private));
	if (NULL == accl_iio_private) {
		err = -ENOMEM;
		goto FREE_IIO_DEVICE;
	}

	gyro_iio_private = iio_allocate_device(sizeof(*bmi160_private));
	if (NULL == gyro_iio_private) {
		err = -ENOMEM;
		goto FREE_IIO_DEVICE;
	}

	magn_iio_private = iio_allocate_device(sizeof(*bmi160_private));
...
FREE_IIO_DEVICE:
	if (accl_iio_private)
		iio_free_device(accl_iio_private);

	if (gyro_iio_private)
		iio_free_device(gyro_iio_private);

	if (magn_iio_private)
		iio_free_device(magn_iio_private);
