...
static void bma2x2_delay(u32 msec)
{
	if (msec <= 20)
...
static void bma2x2_timer_work_fun(struct work_struct *work)
{
	...
	if (bma2x2_get_fifo_framecount(bma2x2->bma2x2_client, &count) < 0) {
		PERR("bma2x2_get_fifo_framecount err\n");
		return;
	}
	if (count == 0) {
		PERR("bma2x2_get_fifo_framecount zero\n");
		return;
	}
	if (count > MAX_FIFO_F_LEVEL) {
		if (bma2x2_get_mode(bma2x2->bma2x2_client, &mode) < 0) {
			PERR("bma2x2_get_mode err\n");
			return;
		}
...
/* please give attation for the fifo output data format*/
	if (f_len == 6) {
		...
		for (i = 0; i < count; i++) {
			if (bma2x2->debug_level & 0x01)
				printk(KERN_INFO "bma time =%llu fifo_time =%llu  bma->count=%llu time_internal =%lld time_odr = %lld ",
				bma2x2->timestamp, bma2x2->fifo_time,
				bma2x2->acc_count, time_internal, time_odr);
			...
			input_event(bma2x2->input, EV_MSC, MSC_TIME,
			ts.tv_sec);
			input_event(bma2x2->input, EV_MSC, MSC_TIME,
			ts.tv_nsec);
			input_event(bma2x2->input, EV_MSC,
...
static ssize_t bma2x2_debug_level_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int err;
	struct i2c_client *client = to_i2c_client(dev);
...
static ssize_t bma2x2_debug_level_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int32_t ret = 0;
	...
	if (ret)
		return ret;
	bma2x2->debug_level = (uint8_t)data;
	return count;
}
...
static DEVICE_ATTR(enable_timer, S_IWUSR|S_IWGRP|S_IWOTH,
		bma2x2_enable_timer_show, bma2x2_enable_timer_store);
static DEVICE_ATTR(debug_level, S_IWUSR|S_IWGRP|S_IWOTH,
		bma2x2_debug_level_show, bma2x2_debug_level_store);
...
static void bma2x2_early_suspend(struct early_suspend *h)
{
	...
	if (data->is_timer_running) {
		...
		if (bma2x2_set_fifo_mode(data->bma2x2_client, 0) < 0)
...
static void bma2x2_late_resume(struct early_suspend *h)
{
	...
	if (data->is_timer_running) {
		...
		if (bma2x2_set_fifo_mode(data->bma2x2_client, 2) < 0)
			PERR("set fifo_mode falied");
		data->base_time = 0;
		data->timestamp = 0;
...
MODULE_DEVICE_TABLE(i2c, bma2x2_of_match);
