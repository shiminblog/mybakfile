	...
	int header, struct bmi160_accel_t acc, u64 t)
{
	...
	if (debug_level & 0x01)
		printk(KERN_INFO "data %s %d %d %d %llu\n",
			...
			t);
#endif
	iio_push_to_buffer(indio_dev->buffer, buf_16, 0);
...
#ifdef BMI160_TIMER_AND_WORKQUEUE

static ssize_t bmi160_enable_timer_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct bmi_client_data *client_data = iio_priv(indio_dev);
...
static ssize_t bmi160_enable_timer_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
...
static IIO_DEVICE_ATTR(selftest, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_selftest_show, bmi160_selftest_store, 0);
static IIO_DEVICE_ATTR(fifo_flush, S_IWUSR|S_IWGRP|S_IWOTH,
		NULL, bmi160_fifo_flush_store, 0);
static IIO_DEVICE_ATTR(fifo_bytecount, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_fifo_bytecount_show, bmi160_fifo_bytecount_store, 0);
static IIO_DEVICE_ATTR(fifo_data_sel, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_fifo_data_sel_show, bmi160_fifo_data_sel_store, 0);
static IIO_DEVICE_ATTR(fifo_watermark, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_fifo_watermark_show, bmi160_fifo_watermark_store, 0);
static IIO_DEVICE_ATTR(fifo_header_en, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_fifo_header_en_show, bmi160_fifo_header_en_store, 0);
static IIO_DEVICE_ATTR(fifo_time_en, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_fifo_time_en_show, bmi160_fifo_time_en_store, 0);
static IIO_DEVICE_ATTR(fifo_int_tag_en, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
...
static IIO_DEVICE_ATTR(acc_range, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_acc_range_show, bmi160_acc_range_store, 0);
static IIO_DEVICE_ATTR(acc_odr, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_acc_odr_show, bmi160_acc_odr_store, 0);
static IIO_DEVICE_ATTR(acc_op_mode, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_acc_op_mode_show, bmi160_acc_op_mode_store, 0);
static IIO_DEVICE_ATTR(acc_value, S_IRUGO,
		bmi160_acc_value_show, NULL, 0);
static IIO_DEVICE_ATTR(acc_fast_calibration_x, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_acc_fast_calibration_x_show,
		bmi160_acc_fast_calibration_x_store, 0);
static IIO_DEVICE_ATTR(acc_fast_calibration_y, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_acc_fast_calibration_y_show,
		bmi160_acc_fast_calibration_y_store, 0);
static IIO_DEVICE_ATTR(acc_fast_calibration_z, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_acc_fast_calibration_z_show,
		bmi160_acc_fast_calibration_z_store, 0);
static IIO_DEVICE_ATTR(acc_offset_x, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_acc_offset_x_show,
		bmi160_acc_offset_x_store, 0);
static IIO_DEVICE_ATTR(acc_offset_y, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_acc_offset_y_show,
		bmi160_acc_offset_y_store, 0);
static IIO_DEVICE_ATTR(acc_offset_z, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_acc_offset_z_show,
		bmi160_acc_offset_z_store, 0);
static IIO_DEVICE_ATTR(test, S_IRUGO,
		bmi160_test_show, NULL, 0);
static IIO_DEVICE_ATTR(stc_enable, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_step_counter_enable_show,
		bmi160_step_counter_enable_store, 0);
static IIO_DEVICE_ATTR(stc_mode, S_IWUSR|S_IWGRP|S_IWOTH,
		NULL, bmi160_step_counter_mode_store, 0);
static IIO_DEVICE_ATTR(stc_clc, S_IWUSR|S_IWGRP|S_IWOTH,
		NULL, bmi160_step_counter_clc_store, 0);
static IIO_DEVICE_ATTR(stc_value, S_IRUGO,
		bmi160_step_counter_value_show, NULL, 0);
static IIO_DEVICE_ATTR(reg_sel, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_show_reg_sel, bmi160_store_reg_sel, 0);
static IIO_DEVICE_ATTR(reg_val, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_show_reg_val, bmi160_store_reg_val, 0);

static IIO_DEVICE_ATTR(driver_version, S_IRUGO,
		bmi160_driver_version_show, NULL, 0);
#ifdef BMI160_DEBUG
static IIO_DEVICE_ATTR(debug_level, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_debug_level_show,
		bmi160_debug_level_store, 0);
#endif
#ifdef BMI160_TIMER_AND_WORKQUEUE
static IIO_DEVICE_ATTR(enable_timer, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_enable_timer_show,
		bmi160_enable_timer_store, 0);
#endif

/* gyro part */
static IIO_DEVICE_ATTR(gyro_op_mode, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
...
static IIO_DEVICE_ATTR(gyro_range, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_gyro_range_show, bmi160_gyro_range_store, 0);
static IIO_DEVICE_ATTR(gyro_odr, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
...
static IIO_DEVICE_ATTR(gyro_offset_x, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
bmi160_gyro_offset_x_show, bmi160_gyro_offset_x_store, 0);
static IIO_DEVICE_ATTR(gyro_offset_y, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
bmi160_gyro_offset_y_show, bmi160_gyro_offset_y_store, 0);
static IIO_DEVICE_ATTR(gyro_offset_z, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
bmi160_gyro_offset_z_show, bmi160_gyro_offset_z_store, 0);

#ifdef BMI160_MAG_INTERFACE_SUPPORT
static IIO_DEVICE_ATTR(mag_op_mode, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_mag_op_mode_show, bmi160_mag_op_mode_store, 0);
static IIO_DEVICE_ATTR(mag_odr, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_mag_odr_show, bmi160_mag_odr_store, 0);
static IIO_DEVICE_ATTR(mag_i2c_addr, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
...
static IIO_DEVICE_ATTR(mag_offset, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_mag_offset_show, bmi160_mag_offset_store, 0);
static IIO_DEVICE_ATTR(mag_chip_id, S_IRUGO,
		bmi160_mag_chip_id_show, NULL, 0);
#endif
#if defined(BMI160_ENABLE_INT1) || defined(BMI160_ENABLE_INT2)
static IIO_DEVICE_ATTR(enable_int, S_IWUSR|S_IWGRP|S_IWOTH,
		NULL, bmi_enable_int_store, 0);
static IIO_DEVICE_ATTR(anymot_duration, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_anymot_duration_show, bmi160_anymot_duration_store, 0);
static IIO_DEVICE_ATTR(anymot_threshold, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
...
static IIO_DEVICE_ATTR(std_en, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_step_detector_enable_show,
		bmi160_step_detector_enable_store, 0);
static IIO_DEVICE_ATTR(sig_en, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
...
#ifdef BMI160_TIMER_AND_WORKQUEUE
	&iio_dev_attr_enable_timer.dev_attr.attr,
#endif
...
static void bmi160_timer_work_fun(struct work_struct *work)
{
	struct bmi_client_data *client_data =
		container_of(work, struct bmi_client_data, report_data_work);
	int err = 0;
	uint32_t fifo_bytecount_tmp = 0;
	...
	err = BMI_CALL_API(fifo_length)(&fifo_bytecount_tmp);
	if (fifo_bytecount_tmp != 0) {
		if (!err) {
			memset(fifo_data, 0, 1024);
			err = bmi_i2c_read(client_data->device.dev_addr,
				BMI160_USER_FIFO_DATA__REG, fifo_data,
	...
	struct hrtimer *hrtimer)
{
	...
	queue_work(reportdata_wq, &(bmi160_private->report_data_work));
	hrtimer_forward(hrtimer, ktime_get(), bmi160_private->work_delay_kt);
...
#ifdef BMI160_TIMER_AND_WORKQUEUE
	INIT_WORK(&bmi160_private->report_data_work,
	bmi160_timer_work_fun);
	reportdata_wq = create_singlethread_workqueue("bmi160_wq");
	if (NULL == reportdata_wq)
		dev_err(p_i2c_dev,
		"create_singlethread_workqueue failed %d\n", -ENOMEM);
	hrtimer_init(&bmi160_private->timer, CLOCK_MONOTONIC,
		HRTIMER_MODE_REL);
	bmi160_private->timer.function = reportdata_timer_fun;
	bmi160_private->work_delay_kt = ns_to_ktime(20000000);
	bmi160_private->is_timer_running = 0;
#endif
