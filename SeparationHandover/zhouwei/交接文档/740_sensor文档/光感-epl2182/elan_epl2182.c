 /* drivers/input/misc/elan_epl2182.c - light and proximity sensors driver
 ...
 *********************************************************/
 ...
 #define LUX_PER_COUNT			440
  #define log(x...)  printk(KERN_ERR x)
 #define  DEBUG
 #ifdef DEBUG
 #define epl_info(fmt, ...) \
 ...
 **********************************************************/
 ...
 struct elan_epl_data {
	...
	struct pinctrl_state *dis_state_default; 
	struct pinctrl *pinctrl;
	struct pinctrl_state * irq_state_default;
 	int intr_pin;
 	int (*power) (int on);
 ...
 */
 			 ...
 			 loff_t *ppos)
 {
	...
	printk(KERN_ERR "%s,%d\n",__func__,__LINE__);
 	if (epld->read_flag == 1) {
		printk(KERN_ERR "%s,%d read data to user space\n",__func__,__LINE__);
 ...
 static int setup_interrupt(struct elan_epl_data *epld)
 {
 	...
 	client->irq = gpio_to_irq(epld->irq_gpio); 
 	err =
 			     ...
 			     const struct i2c_device_id *id)
 {
	...
	if (IS_ERR(epld->pinctrl)) 
	{ 
		pr_err("%s:failed to get pinctrl\n", __func__);
		return -EINVAL;
	}
	epld->irq_state_default = pinctrl_lookup_state(epld->pinctrl, "epl2182_int_active");
	if (IS_ERR(epld->irq_state_default)) 
	{ 
		pr_err("%s:can not get irq pinstate\n", __func__);
		return -EINVAL; 
	}
	err = pinctrl_select_state(epld->pinctrl, epld->irq_state_default); 
	if (err)
	{ 
		pr_err("%s:set irq pin state failed!\n", __func__);
	}
	epld->irq_gpio = of_get_named_gpio_flags(client->dev.of_node, "epl2182,irq-gpio", 0, NULL);
	printk("epl2182 irq gpio = %d\n",epld->irq_gpio);
	#endif
