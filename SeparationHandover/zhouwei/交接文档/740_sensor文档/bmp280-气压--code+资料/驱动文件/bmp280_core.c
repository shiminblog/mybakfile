...
struct bmp_client_data {
	...
	struct mutex lock;
	/*!sensor classdev */
	struct sensors_classdev pressure_cdev;
 ...
 * @retval 4000 4000ms
*/
static uint32_t bmp_get_standbydur(struct bmp_client_data *data)
{
	int err = 0;
	uint8_t standbydur;
 ...
 * @retval non-zero failed
*/
static int bmp_set_standbydur(struct bmp_client_data *data, uint32_t standbydur)
{
	int err = 0;
	int i;
 ...
 * @retval non-zero failed
*/
static int bmp_set_workmode(struct bmp_client_data *data, uint8_t workmode)
{
	int err = 0;
	int i;
...
//<-- 2016-05-09@zhouwei 
static int bmp280_enable_set(struct sensors_classdev *sensors_cdev,
						unsigned int enable)
//--!>
{
	struct bmp_client_data *data = container_of(sensors_cdev,
					struct bmp_client_data, pressure_cdev);
...
//<-- 2016-05-09@zhouwei 
static int bmp280_poll_delay_set(struct sensors_classdev *sensors_cdev,
			...
			const char *buf, size_t count)
{
	...
	if (status == 0) {
		bmp280_poll_delay_set(&data->pressure_cdev, (uint32_t)delay);
		return count;
	}
	return status;
}
			...
			const char *buf, size_t count)
{
	...
	if (status == 0) {
		enable = enable ? 1 : 0;
		bmp280_enable_set(&data->pressure_cdev, (uint32_t)enable);
		return count;
	}
	return status;
}
...
int bmp_probe(struct device *dev, struct bmp_data_bus *data_bus)
{
	...
	data->pressure_cdev = sensors_cdev;
	data->pressure_cdev.sensors_enable = bmp280_enable_set;
	data->pressure_cdev.sensors_poll_delay = bmp280_poll_delay_set;
	err = sensors_classdev_register(&data->input->dev, &data->pressure_cdev);
	if (err) {
		dev_err(dev, "Sensor class device create failed: %d\n", err);
		goto error_sysfs;
	}
